...
std::unique_ptr<matlab::engine::MATLABEngine> start_matlab(const char *sdp_solver_folder) {
    ...
    // Set path
    std::string sdp_folder(sdp_solver_folder);
    std::string sdp_matlab_path = std::string("addpath(genpath('" + sdp_folder + "'));");
    std::u16string sdp_path_command(sdp_matlab_path.begin(), sdp_matlab_path.end());
