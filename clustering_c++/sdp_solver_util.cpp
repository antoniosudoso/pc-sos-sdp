...
arma::sp_mat build_A_cannot_link(arma::sp_mat &old_A, std::vector<std::pair<int, int>> &local_cl_pairs) {
    int m = old_A.n_rows;
    int n = old_A.n_cols;
	int n_pairs = local_cl_pairs.size();
    arma::sp_mat A = arma::resize(old_A, m + n_pairs, n);
    // Z_ij = 0
    int sqrt_n = std::sqrt(n);
	for (int k = 0; k < n_pairs; k++){
		int i = local_cl_pairs[k].first;
		int j = local_cl_pairs[k].second;
		A(m + k, sqrt_n * i + j) = 0.5;
		A(m + k, sqrt_n * j + i) = 0.5;
	}
    return A;
}

arma::sp_mat build_A(std::map<int, std::set<int>> &ml_map,
						std::vector<std::pair<int, int>> &local_cl_pairs) {
	int n = ml_map.size();

	arma::sp_mat TTt = build_TTt(ml_map);
    arma::sp_mat A = build_A_must_link(n+1, TTt);

	A = build_A_cannot_link(A, local_cl_pairs);
    return A;
}

arma::vec build_b(int k, std::map<int, std::set<int>> &ml_map,
					std::vector<std::pair<int, int>> &local_cl_pairs) {

	int n = ml_map.size();
    arma::vec b = build_b_must_link(n+1, k);

    for (auto &elem : local_cl_pairs) {
        b = build_b_cannot_link(b);
    }
    return b;
}
