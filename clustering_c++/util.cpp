...
//build Ws given the original Ws matrix and a ml_map
arma::mat build_Ws(arma::mat &old_Ws, std::map<int, std::set<int>> &ml_map){

	int n = ml_map.size();
	arma::mat T = arma::zeros(n, old_Ws.n_rows);
	for (int k = 0; k < n; k++){
        for (auto &elem : ml_map[k]) {
            T(k, elem) = 1;
        }
	}
	return (T * old_Ws);
...
double heuristic_solve(arma::mat &data, int k, bool verbose, int n_start, int max_iter, int n_permutations,
                    	...
                    	arma::mat &distances, arma::sp_mat &assignment_X, arma::mat &out_centroids) {
    // call constrained k-means
    Kmeans kmeans(data, k, ml_map, local_cl, global_ml, global_cl, verbose);
    bool flag_partition = kmeans.start(max_iter, n_start, n_permutations, distances);
    if (!flag_partition)
...
void print_log_sdp(std::ostream &log_file, int n, int node_parent, int node, double lb_parent, double lb,
                   ...
                   double ub, double gub, int i, int j, double node_gap, double gap, int open, bool update) {
...
int have_same_assignment(arma::sp_mat &assignment_X, int i, int j,
									std::map<int, std::set<int>> &ml_map){
	int global_i = *(ml_map[i].begin());
	int global_j = *(ml_map[j].begin());
	return arma::approx_equal(assignment_X.row(global_i),
								assignment_X.row(global_j),
								"absdiff", 0.001);
}
